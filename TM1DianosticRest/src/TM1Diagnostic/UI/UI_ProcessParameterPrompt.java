...
public class UI_ProcessParameterPrompt extends Dialog {
	...
	private Button notify_checkbutton;
	
	/**
	 * @wbp.parser.constructor
	 */
	public UI_ProcessParameterPrompt(Shell parent, TM1Process process, ServerExplorerComposite parentUI) {
	...
	public UI_ProcessParameterPrompt(Shell parent, TM1Process process) {
		super(parent, SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL);
		setText("SWT Dialog");
		this.process = process;
	...
	private void click_execute_new_thread(){
		for (int i=0; i<process.parametercount(); i++){
			String value = table.getItem(i).getText(EDITABLECOLUMN);
			process.getparameter(i).setvalue(value);
		}
		
		Thread runti = new Thread(new ProcessWorker(process, parentUI, notify_checkbutton.getSelection()));
		runti.start();
	...
	private void createContents() {
		...
		shlProcessParameters.setSize(775, 294);
		shlProcessParameters.setText("Process Parameters");
		...
		tblclmnNewColumn_3.setWidth(56);
		...
		notify_checkbutton = new Button(composite_1, SWT.CHECK);
		notify_checkbutton.setText("Notify");
		
		Button btnNewButton = new Button(composite_1, SWT.NONE);
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent arg0) {
				click_execute_new_thread();
			}
		});
